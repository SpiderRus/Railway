...
class Circle {
     ...
     */
    drawDangerZone(ctx, angles, offset, firstNMarks=0, lastNMarks=0) {
     ...
     */
    drawMark(ctx, x1, y1, x2, y2, lineWidth, strokeStyle) {
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.closePath();
     ...
     */
    drawMarks(ctx, angles, length, lineWidth, border=true, offset=0, firstNMarks=0, lastNMarks=0, skip=null, skipFrom=0, skipTo=0) {
     ...
     */
    drawTextMarks(ctx, angles, length, step, offset, fractions=false, reverse=false, start = 0, correctZero = 0) {
        let value = start;
        let res = 0;        
        const radius = this.radius - this.circleBorderWidth - offset;
        ...
        for (let angle in angles) {
            //console.log('a=' + this.textMarksOffsetX);
            ...
            if (Math.abs(value) > 0.001 && fractions) {
                let f = new Fraction(value);
                res = `${f.n}/${f.d}`;
                this.drawTextMark(ctx, res, x, y);
            } else {
        	if (value > -0.001)
        	    this.drawTextMark(ctx, value, x + correctZero, y);
        	else
     ...
     * @param {number} [startAngle=0] The angle (in degrees) from which to start the calculation.
     */
    drawArrow(ctx, value, degAngle, offset=0, startAngle=0, correction=0) {
	value += correction;
     ...
     */
    drawTurnSignal(ctx, offsetX, offsetY, width, height, enabled=false) {
        ctx.save();
        ctx.translate(this.x, this.y);
        ctx.beginPath();
     ...
     */
    drawMileage(ctx, value, offsetX, offsetY, width, height, radius) {
     ...
     * @param {number} offsetY Offset in px from center of circle by y-coordinate.
     */
    drawMultiplier(ctx, multiplier, offsetY) {
 ...
 */
drawLeftCircle(radius, value, iconStates) {
    const lCircle = new AdditionalCircle(this.options, 'left', radius);
    lCircle.draw(this.ctx);

    const addMarksAngles = lCircle.calcMarksAngles(8, 140, 300);

    const mainMarksAngles = lCircle.calcMarksAngles(9, 160, 300);

    lCircle.drawDangerZone(this.ctx, addMarksAngles, 6, 0, 2);

    lCircle.drawDangerZone(this.ctx, mainMarksAngles, 6, 0, 2);

    lCircle.drawMarks(this.ctx, addMarksAngles, 7.5, 2, true, 1, 0, 2);

    lCircle.drawMarks(this.ctx, mainMarksAngles, 6, 3, true, 0, 0, 2);
    
    const textMarksAngles = lCircle.calcTextMarksAngles(9, 160, 5, 300);

    lCircle.drawTextMarks(this.ctx, textMarksAngles, 6, 1, 12.5);

    lCircle.drawIcon(this.ctx, 'battery', iconStates['battery']);
    lCircle.drawIcon(this.ctx, 'oil', iconStates['oil']);
    lCircle.drawIcon(this.ctx, 'engineFail', iconStates['engineFail']);

    lCircle.drawMultiplier(this.ctx, 1, -20);

    lCircle.drawArrow(this.ctx, value, 160, 5, 300);

    lCircle.drawArrowBody(this.ctx);
}
 ...
 * @param {Object} iconStates States of icons displayed on circle.
 */
drawRightCircle(radius, value, iconStates) {
    const rCircle = new AdditionalCircle(this.options, 'right', radius);
    rCircle.draw(this.ctx);

    const addMarksAngles = rCircle.calcMarksAngles(8, 140, 80);

    const mainMarksAngles = rCircle.calcMarksAngles(9, 160, 80);

    rCircle.drawDangerZone(this.ctx, addMarksAngles, 6, 0, 2);

    rCircle.drawDangerZone(this.ctx, mainMarksAngles, 6, 0, 2 );

    rCircle.drawMarks(this.ctx, addMarksAngles, 7.5, 2, true, 1, 0, 2, 'all', 0, 5);

    rCircle.drawMarks(this.ctx, mainMarksAngles, 6, 3, true, 0, 0, 2, 'even', 0, 6);
    
    const textMarksAngles = rCircle.calcTextMarksAngles(3, 160, -5, 80);

    rCircle.drawTextMarks(this.ctx, textMarksAngles, 6, 0.5, 17.5, true, true);

    rCircle.drawIcon(this.ctx, 'gas', iconStates['gas']);
    rCircle.drawIcon(this.ctx, 'trunk', iconStates['trunk']);
    rCircle.drawIcon(this.ctx, 'bonnet', iconStates['bonnet']);
    rCircle.drawIcon(this.ctx, 'doors', iconStates['doors']);

    rCircle.drawArrow(this.ctx, Math.abs(value - 1), 160, 5, 80);

    rCircle.drawArrowBody(this.ctx);
}
...
draw(speedometerValue, tachometerValue, gasValue, mileage, turnSignals, iconStates) {
    ...
    const mCircle = new MainCircle(this.options);
    this.drawLeftCircle(mCircle.radius, tachometerValue, iconStates);
    this.drawRightCircle(mCircle.radius, gasValue, iconStates);
    
    mCircle.draw(this.ctx);

    var m = this.options.maxSpeed / 20.0;
    var max = Math.round(m);
    if (m - max > 0.01)
	    max = max + 1;
    ...
    mCircle.drawMarks(this.ctx, addMarksAngles, 7.5, 2, true, 4, 0, 0);

    mCircle.drawMarks(this.ctx, mainMarksAngles, 6, 3, true, 3);

    const textMarksAngles = mCircle.calcTextMarksAngles(max + 1, 200, 7.5);

    mCircle.drawTextMarks(this.ctx, textMarksAngles, 6, 20, 22.5, false, false, -10 * (max), 7);

    mCircle.drawTurnSignal(this.ctx, 35, -25, 25, 15, turnSignals['right']);
    mCircle.drawTurnSignal(this.ctx, -35, -25, -25, 15, turnSignals['left']);

    mCircle.drawIcon(this.ctx, 'dippedBeam', iconStates['dippedBeam']);
    mCircle.drawIcon(this.ctx, 'brake', iconStates['brake']);
    mCircle.drawIcon(this.ctx, 'drift', iconStates['drift']);
    mCircle.drawIcon(this.ctx, 'highBeam', iconStates['highBeam']);
    mCircle.drawIcon(this.ctx, 'lock', iconStates['lock']);
    mCircle.drawIcon(this.ctx, 'seatBelt', iconStates['seatBelt']);
    mCircle.drawIcon(this.ctx, 'engineTemp', iconStates['engineTemp']);
    mCircle.drawIcon(this.ctx, 'stab', iconStates['stab']);
    mCircle.drawIcon(this.ctx, 'abs', iconStates['abs']);

    mCircle.drawMileage(this.ctx, mileage, 0, 60, 100, 20, 2);
    
    mCircle.drawUnit(this.ctx, -25);
    
    mCircle.drawArrow(this.ctx, (speedometerValue / 2 + 0.5), 200, 10);

    mCircle.drawArrowBody(this.ctx);
}
}
